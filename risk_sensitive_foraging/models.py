...
# self.session.vars['PM'] = exp.pm
# self.session.vars['AM'] = exp.am
...
class Subsession(BaseSubsession):
    ...
    for p in self.get_players():
      ...
      if (self.round_number == 1):
        ...
        # p.colorx1_x2 = random.randint(0, 1)
          ...
          # self.session.vars['num_blocks'] = self.session.vars['PM'].blocks[self.session.vars['PM'].i]

          # # Predefine random outcomes of all options in all trials
          # p.participant.vars['outcomes'] = [ [ p.draw_outcomes(gamble, self.session.vars['PM'].trials[self.session.vars['PM'].i] + 1) for gamble in a] for a in rnd_actions]
  ...
  #     return(False)
  #   else:
  #     return((self.session.vars['PM'].get_block(round_number - 1) != self.session.vars['PM'].get_block(round_number)))
